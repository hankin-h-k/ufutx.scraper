...
Route::middleware('auth:api')->group(function () {
     ...
     * 图书分类
     */
    Route::get('/libraries/{library_id}/sorts', 'SortController@librarySorts');
    Route::post('/libraries/{library_id}/sorts', 'SortController@librarySortStore');
    ...
    * 图书馆管理
    */
    Route::get('/library/sort/init', 'LibraryController@initLibrarySort');
     ...
     * 图书管理
     */
    Route::post('/user/books/store', 'BookController@userBooksStore');
    Route::post('/libraries/{id}/books/store', 'BookController@libraryBooksStore');
    Route::get('/user/books', 'BookController@userBooks');
    Route::get('/users/{user_id}/books', 'BookController@userBooks');
    Route::get('/libraries/{library_id}/books', 'BookController@libraryBooks');
    Route::get('/libraries/{library_id}/sorts/{sort_id}/books', 'BookController@librarySortBooks');
    Route::post('/libraries/{library_id}/books', 'BookController@libraryBookStore');
    Route::get('/libraries/{library_id}/books/{id}', 'BookController@libraryBook');
    Route::put('/libraries/{library_id}/books/{id}', 'BookController@libraryBookUpdate'); 
    Route::delete('/libraries/{library_id}/books/{id}', 'BookController@libraryBookDelete'); 
    Route::get('/books', 'BookController@books'); 
    Route::post('/books/v2', 'BookController@bookStoreV2');
    Route::post('/books', 'BookController@bookStore');//仅添加图书
    Route::get('/books/{id}', 'BookController@item')->where('id', '[0-9]+');
    Route::get('/books/{id}/v2', 'BookController@itemV2')->where('id', '[0-9]+');
    Route::get('/books/{id}/comments', 'BookController@comments')->where('id', '[0-9]+');
    Route::post('/books/{id}/comments', 'BookController@commentStore')->where('id', '[0-9]+');
    ...
    //todo:老接口,待更新
    Route::put('/books/{id}', 'HomeController@bookUpdate')->where('id', '[0-9]+'); //absolete
    Route::delete('/books/{id}', 'HomeController@bookDestory')->where('id', '[0-9]+'); //absolete
    ...
    Route::delete('/libraries/{library_id}/borrows/{id}', 'BorrowController@deleteBorrow');
    //isbn查借阅
    Route::get('/libraries/{library_id}/borrow/isbn/{isbn}', 'BorrowController@borrowIsbn');
    //isbn查书
    Route::get('/libraries/{library_id}/book/isbn/{isbn}', 'BorrowController@bookIsbn');
    ...
    Route::get('/users/{user_id}', 'UserController@user');
    Route::put('/users/{user_id}/follow', 'UserController@userFollow');
    Route::get('/users/{user_id}/followers', 'UserController@userFollowers');
    Route::get('/users/{user_id}/followings', 'UserController@userFollowings');
    Route::get('/user/borrows', 'UserController@borrows');
    Route::get('/user/libraries', 'UserController@libraries');
});
